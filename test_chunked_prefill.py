...
class TestDecodePhase(TestCase):
    ...
    def decode_phase(
        ...
        # This step is just ... no use
        q_lens = context_lens[1:] 
        q_lens = [random.randint(1, 1) for max_lens in q_lens]
        q_lens = [0] + q_lens
        q_lens_tensor = torch.tensor(q_lens, dtype=torch.int, device="cpu")

        # Our purpose is to get cu_seqlen_q and cu_seqlen_k
        ...
        # WARNING: this context_lens_xpu has errors
...
class TestChunkedPrefill(TestCase):
    ...
    def chunk_prefill(
        ...
        if not use_v2:
            ...
            out = vllm._C.ops.context_attention_forward_v2(query_xpu, key_cache_xpu, value_cache_xpu, block_tables_xpu, cu_seqlen_q_xpu, seq_lens_tensor_xpu , context_lens_tensor_xpu, max_seqlen_k, torch.amax(context_lens_tensor_xpu).item(), torch.amax(q_lens_tensor_xpu).item())
...
instantiate_parametrized_tests(TestDecodePhase)
